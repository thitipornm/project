<template>
  <div>
    <h1>Create Food</h1>
    <form v-on:submit.prevent="createFood">
      <p>ชื่ออาหาร : <input type="text" v-model="food.foodname" /></p>
...
export default {
  ...
  methods: {
    async createFood() {
      try {
        this.food.pictures = JSON.stringify(this.pictures);
        await FoodsService.post(this.food);
        this.$router.push({
          name: "foods",
        });
      } catch (err) {
